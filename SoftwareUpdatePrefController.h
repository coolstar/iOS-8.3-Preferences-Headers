...
@class UsageStorageController, NSString, SUDownload, NSDateComponentsFormatter, SUManagerClient, UINavigationController, NSMutableArray, RUILoader, NSURL, NSError, PSSoftwareUpdateTitleCell, SUMutableDownloadMetadata, UIAlertView, PSSpecifier, SUDescriptor;

__attribute__((visibility("hidden")))
@interface SoftwareUpdatePrefController : XXUnknownSuperclass <DevicePINControllerDelegate, SUManagerClientDelegate, UITableViewDelegate, SUNetworkObserver, RUIObjectModelDelegate> {
...
@property(copy, nonatomic) id devicePasscodeCompletion;
...
-(void)showStorageHelpLinkIfNeededOnFooterForGroup:(id)group error:(id)error;
-(BOOL)handleStoragePurgeIfNeeded;
-(void)storagePurgeFinished:(BOOL)finished;
-(void)install:(id)install;
-(void)_reallyInstall;
-(void)downloadAndInstall:(id)install;
-(void)promptForDevicePasscodeFromSpecifier:(id)specifier withCompletion:(id)completion;
...
-(void)setShowsTermsProgress:(BOOL)progress;
